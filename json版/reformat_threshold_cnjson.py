...
"""
================================================================================
【留存脚本】国内/国际天气数据一致性比对 -- 旧版（国内 coapi JSON 接口 vs 国际 JSON 接口）
================================================================================
⚠️ 这是「之前版本」的留存脚本（git HEAD bc0046c, 2026-07-24, 切 proto 前最后一个完整 json 版）。
   当前线上比对已切换为 proto 版本（工作区未提交）：
   - 当前版本：utils/reformat_threshold.py + utils/fetch_cn_pb.py + utils/raw_pull.py
     · 国内 = weather.api.moji.com/data/detail（POST, protobuf 二进制）
     · 国际 = datasw1.api.moweather.com/whapi/in/weather（GET, json, 含 nowcast 短时降水）
   - 本留存版本（旧）：
     · 国内 = coapi.moji.com/whapi/v2/weather（GET, MD5 鉴权, json）
     · 国际 = datasw1.api.moweather.com/whapi/in/weather（GET, HMAC-SHA256 鉴权, json）
     · 配置驱动（本目录 compare_config_cnjson.yaml，与现在同源体系）

口径（与 proto 版同体系, 仅国内数据源不同）：
  · 阈值口径: 温度2/体感2/湿度10/风速2/气压10/AQI20（|国内-海外|≤阈值算一致）
  · 天气现象: weather_mapping 44+29种语义映射, 5分制, score≥4算一致（对齐老报告口径）
  · 降水等级: rain_thresholds 7级（无雨或零星小雨/小雨/中雨/大雨/暴雨/大暴雨/特大暴雨）, 等级相同算一致
  · 风速: 海外÷3.6换算 m/s
  · 双报告: 严格相等 + 阈值口径 各一份 xlsx
  · 天气现象按中文文字比对（国内 coapi 返回中文 weather, 国际 lang=zh-CN）

与 proto 版的差异：
  1. 国内数据源: coapi json 接口 vs proto detail 接口（字段映射同构, normalize_cn 转 pb 为同结构）
  2. 天气现象一致判定: 本版 score≥4（对齐老报告）; proto 版 score≥5
  3. 降水概率: 本版无; proto 版新增 pop 对比
  降水等级均 7 级、weather_mapping texts(44+29种) 完全一致。

留存用途：作为后续可能复用的「国内 coapi json 接口 vs 国际 json 接口」比对方案参考。
         运行会真实拉接口，建议自行手动执行，勿后台无人值守触发。
运行：python3 utils/legacy/reformat_threshold_cnjson.py
配套配置：utils/legacy/compare_config_cnjson.yaml（本目录, 勿与项目根 proto 版 config 混用）
输出：data/比对结果/legacy_json接口版/{一致性比对报告_严格相等_*.xlsx, ..._阈值口径_*.xlsx, 数据明细_*.csv}
================================================================================
...
OUT_DIR = os.path.join(BASE, '比对结果', 'legacy_json接口版')   # 独立输出目录, 避免覆盖 proto 版
...
CONFIG_PATH = os.path.join(SCRIPT_DIR, 'compare_config_cnjson.yaml')   # 指向本目录配套 config
