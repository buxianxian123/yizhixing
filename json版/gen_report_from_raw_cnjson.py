...
数据源: data/原始拉取_json/原始_<时间戳>/<城市>/国内.json + 国际.json（raw_pull_cnjson.py 拉取留底）
加工链路与 scheduled_compare.py 完全一致，只是手动取最近N份一次性出报告（不用等定时任务攒满）。
raw_pull_cnjson 每跑一轮都留底 原始拉取_json/原始_<时间戳>/<城市>/，本脚本读这些留底复现均值报告。
...
import scheduled_compare_cnjson as sc
...
def main():
    ...
    print(f"扫描 raw_pull_cnjson 留底: {RAW_PULL_DIR}")
    pulls = load_raw_pulls(cities)
    print(f"找到 {len(pulls)} 份拉取: {', '.join(p[0] for p in pulls)}")
    if not pulls:
        print(f"❌ 未找到原始拉取，请确认 {RAW_PULL_DIR}/原始_* 目录存在（需先跑 raw_pull_cnjson.py 拉取）")
        return
    ...
    extra = [
        f'本报告从 raw_pull_cnjson 留底复现（验证加工逻辑）: 取 {avg_count} 份原始拉取做均值',
        f'采样窗口: {window_start} ~ {window_end}',
        '数据源: data/原始拉取_json/原始_<ts>/<城市>/国内.json + 国际.json (raw_pull_cnjson.py 拉取)',
