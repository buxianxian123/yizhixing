...
# -*- coding: utf-8 -*-
"""
定时均值比对 - json版(coapi)（常驻脚本，读留底不拉接口）
...
⚠️ 本脚本不拉接口！原始数据由 raw_pull.py 拉取存到 data/原始拉取/，本脚本读这些留底攒均值。
   需先跑 raw_pull.py 持续拉取: nohup python3 json版/raw_pull_cnjson.py > raw_pull.log 2>&1 &
   手动取最近N份出报告(不等攒满): python3 proto版/gen_report_from_raw_cnjson.py 45
...
import reformat_threshold_cnjson as rt   # 复用 build_points/load_cities/gen_xlsx/calc_weather_deviation/get_threshold/MODULES

# ====== 路径配置 ======
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
OUT_DIR = rt.OUT_DIR                       # data/比对结果
STATE_PATH = os.path.join(OUT_DIR, '.均值累积状态.json')
RAW_PULL_DIR = os.path.join(rt.BASE, '原始拉取_json')   # data/原始拉取 (raw_pull留底)
...
def emit_reports(acc, avg_count, window_start, window_end):
    ...
    extra = [
        ...
        '数据源: data/原始拉取_json/原始_<ts>/<城市>/国内.json + 国际.json (raw_pull_cnjson.py 拉取留底, 本脚本只读不拉)',
...
def main():
    ...
    print(f"  ⚠️ 需 raw_pull_cnjson.py 在跑才有新数据(coapi); 手动出报告用 gen_report_from_raw.py N")
    if MAX_WINDOWS:
